...
String IF97duino_error;

void raise_IF97duino_error(String error_msg) {
  IF97duino_error=error_msg;
}
...
namespace IF97
{
    ...
    public:
        ...
        double rhomass(double T, double p) const{
            return p_star/(R*T)/(p_fact/1000.0/R_fact)/(dgamma0_dp_i(T,p) + dgammar_dp_i(T,p));
        }
        ...
        double umass(double T, double p) const{
            const double tau = T_star/T, p_i = p/p_star;
            return R*T*(tau*(dgamma0_dTAU(T, p) + dgammar_dTAU(T, p)) - p_i*(dgamma0_dp_i(T, p) + dgammar_dp_i(T, p)));
        }
        ...
        virtual double cvmass(double T, double p) const{
            const double tau = T_star/T, p_i = p/p_star;
            return cpmass(T,p)-R*powi(1 + p_i*dgammar_dp_i(T,p) - tau*p_i*d2gammar_dp_idTAU(T,p),2)/(1-p_i*p_i*d2gammar_dp_i2(T, p));
        }
        virtual double speed_sound(double T, double p) const{
            const double tau = T_star/T, p_i = p/p_star;
            const double RHS = (1 + 2*p_i*dgammar_dp_i(T,p) + p_i*p_i*powi(dgammar_dp_i(T,p),2))/((1-p_i*p_i*d2gammar_dp_i2(T,p)) +powi(1 + p_i*dgammar_dp_i(T,p) - tau*p_i*d2gammar_dp_idTAU(T,p), 2)/(tau*tau*(d2gamma0_dTAU2(T,p) + d2gammar_dTAU2(T,p))));
        ...
        virtual double drhodp(double T, double p) const{
            ...
            const double p_i = p/p_star;
            return (rhomass(T,p)/p) * ( (1.0 - p_i*p_i*d2gammar_dp_i2(T,p)) / (1.0 + p_i*dgammar_dp_i(T,p)) );
        }
        ...
        virtual double p_irterm(double) const = 0;
        ...
        double output(IF97parameters key, double T, double p) const{
            ...
            case IF97_Q: raise_IF97duino_error("Can't determine Q from T & P"); return(0);  // just in case
            }
            raise_IF97duino_error("Unable to match input parameters"); return(0);
        }
    ...
    protected:
        ...
        double gammar(double T, double p) const{
            const double _p_i = p_irterm(p), _TAU = TAUrterm(T);
            double summer = 0;
            for (std::size_t i = 0; i < Jr.size(); ++i){
                summer += nr[i]*powi(_p_i, Ir[i])*powi(_TAU, Jr[i]);
            }
            return summer;
        }
        double dgammar_dp_i(double T, double p) const{
            const double _p_i = p_irterm(p), _TAU = TAUrterm(T);
            double summer = 0;
            for (std::size_t i = 0; i < Jr.size(); ++i){
                summer += nr[i]*Ir[i]*powi(_p_i, Ir[i]-1)*powi(_TAU, Jr[i]);
            }
            return summer;
        }
        double d2gammar_dp_i2(double T, double p) const{
            const double _p_i = p_irterm(p), _TAU = TAUrterm(T);
            double summer = 0;
            for (std::size_t i = 0; i < Jr.size(); ++i){
                summer += nr[i]*Ir[i]*(Ir[i]-1)*powi(_p_i, Ir[i]-2)*powi(_TAU, Jr[i]);
            }
            return summer;
        }
        double dgammar_dTAU(double T, double p) const{
            ...
            double summer = 0;
            for (std::size_t i = 0; i < Jr.size(); ++i){
                summer += nr[i]*Jr[i]*powi(_p_i, Ir[i])*powi(_TAU, Jr[i]-1);
            }
            return summer;
        }
        double d2gammar_dp_idTAU(double T, double p) const{
            const double _p_i = p_irterm(p), _TAU = TAUrterm(T);
            double summer = 0;
            for (std::size_t i = 0; i < Jr.size(); ++i){
                summer += nr[i]*Jr[i]*Ir[i]*powi(_p_i, Ir[i]-1)*powi(_TAU, Jr[i]-1);
            }
            return summer;
        }
        double d2gammar_dTAU2(double T, double p) const {
            ...
            double summer = 0;
            for (std::size_t i = 0; i < Jr.size(); ++i){
                summer += nr[i]*Jr[i]*(Jr[i]-1)*powi(_p_i, Ir[i])*powi(_TAU, Jr[i]-2);
            }
            return summer;
        }
        double gamma0(double T, double p) const{
            if (J0.size() == 0){ return 0; } // Region 1 has no term
            const double p_i = p/p_star, _TAU = TAU0term(T);
            double summer = log(p_i);
            for (std::size_t i = 0; i < n0.size(); ++i){
                summer += n0[i]*powi(_TAU, J0[i]);
            }
            return summer;
        }
        double dgamma0_dp_i(double /*T*/, double p) const{
            if (J0.size() == 0){ return 0; } // Region 1 has no term
            const double p_i = p/p_star;
            return 1.0/p_i;
        }
        double d2gamma0_dp_i2(double /*T*/, double p) const{
            if (J0.size() == 0){ return 0; } // Region 1 has no term
            const double p_i = p/p_star;
            return -1.0/(p_i*p_i);
        }
        ...
        double lambda2(double T, double p, double rho) const{
            ...
            if (y < 1.2E-7)                            /// Z is not calculated if y < 1.2E-7 since the
                ...
                Z = 2.0/p_i/y*(((1.0-1.0/k)*atan(y)+y/k) - (1.0 - exp(-1.0/(1.0/y + y*y/(3.0*rhobar*rhobar)))));
    ...
    static const RegionResidualElement Region1residdata[] = {
        // Note: the coefficients of n_i have been multiplied by -1**I_i such that all Gibbs terms are of the form (p_i-7.1)**(I_i) rather than (7.1-p_i)**(I_i)
    ...
    public:
        ...
        double speed_sound(double T, double p) const{
			...
			// see Table 3
            const double tau = T_star/T;
            const double RHS = powi(dgammar_dp_i(T,p), 2)/(powi(dgammar_dp_i(T,p)-tau*d2gammar_dp_idTAU(T,p), 2)/(tau*tau*d2gammar_dTAU2(T,p)) - d2gammar_dp_i2(T, p));
            return sqrt(R*(1000/R_fact)*T*RHS);
        }
        double cvmass(double T, double p) const{
            ...
            // see Table 3
            const double tau = T_star / T;
            return R*(-tau*tau*d2gammar_dTAU2(T,p) + powi(dgammar_dp_i(T, p) - tau*d2gammar_dp_idTAU(T, p), 2) / d2gammar_dp_i2(T, p));
        }
        double drhodp(double T, double p) const{
            ...
            return -d2gammar_dp_i2(T,p)/(powi(dgammar_dp_i(T,p),2)*R*T)*(1000*R_fact/p_fact);
        }
        double TAUrterm(double T) const{
            return T_star/T - 1.222;
        }
        double p_irterm(double p) const{
            return p/p_star - 7.1;
        }
    ...
    inline double Region23_T(double T){
        ...
        const double p_i = region23_n[0] + region23_n[1]*theta + region23_n[2]*theta*theta;
        return p_i*p_star;
    }
    inline double Region23_p(double p){
        const double p_star = 1*p_fact, T_star = 1, p_i = p/p_star;
        const double THETA = region23_n[3] + sqrt((p_i - region23_n[4])/region23_n[2]);
        return THETA*T_star;
    }
    ...
    namespace Region3Backwards{
        ...
        public:
            ...
            virtual double v(double T, double p) const{
                const double p_i = p/p_star, theta = T/T_star;
                double summer = 0;
                for (std::size_t i = 0; i < N; ++i){
                    summer += n[i]*powi(std::pow(p_i-a, c), I[i])*powi(std::pow(theta-b, d), J[i]);
                }
                return std::pow(summer, e)*v_star;
            };

            // This function p_iggybacks off of the Backwards structure already written
            ...
            virtual double Y(double p, double X) const{
                const double p_i = p/p_star, eta = X/X_star;
                double summer = 0;
                for (std::size_t i = 0; i < N; ++i){
                    summer += n[i]*powi(std::pow(p_i-a, c), I[i])*powi(std::pow(eta-b, d), J[i])*powi(f, J[i]);
                }
        ...
        public:
            ...
            double v(double T, double p) const{
                ...
                double summer = 0;
                for (std::size_t i = 0; i < N; ++i){
                    summer += n[i]*powi(p_i-a, I[i])*powi(theta-b, J[i]);
                }
        ...
        inline double Region3_v_TP(char region, double T, double p){
            ...
            switch(region){
                ...
                default:
                    raise_IF97duino_error("Unable to match region"); return(0);
            }
        }
        ...
        public:
            ...
            virtual double T_p(double p) const{
                const double p_i = p/(1.0*p_fact);
                double summer = 0;
                for (std::size_t i = 0; i < N; ++i){
                    summer += n[i]*powi(p_i, I[i]);
                }
        ...
        class ABline : public Region3RegionDivision{ 
            ...
            virtual double T_p(double p) const{
                const double p_i = p/(1.0*p_fact), ln_p_i = log(p_i);
                double summer = 0;
                for (std::size_t i = 0; i < N; ++i){
                    summer += n[i]*powi(ln_p_i, I[i]);
                }
                return summer*1.0;  // sum is multiplied by T* = 1.0 [K]
            };
        };
        ...
        public:
            double T_p(double p) const{ 
                const double p_i = p/(1.0*p_fact); 
                return 3.727888004*(p_i - 22.064) + 647.096; 
            }; 
        };
        ...
        inline double DividingLine(DividingLineEnum region, double p){
            ...
            switch(region){
                ...
                default:
                    raise_IF97duino_error("Unable to match dividing line"); return(0);
            }
        }
        ...
        inline char BackwardsRegion3SubRegionDetermination(double T, double p){
            ...
            if (p > 22.5*p_fact){
               raise_IF97duino_error("Out of range"); return(0);
            }
        ...
        inline char BackwardsRegion3RegionDetermination(double T, double p){
            if (p > 100*p_fact){
                raise_IF97duino_error("pressure out of range"); return(0);
            }
    ...
    public:
        ...
        double lambda2(double T, double /*p*/, double rho) const{
            ...
            const double p_i     = 2*acos(0.0);   /// Have to define this in C++
        ...
        double output(IF97parameters key, double T, double p, IF97SatState State) const{
            ...
            switch(key)                 // return all properties using the new rho value
            {
                ...
                default:
                    raise_IF97duino_error("Bad key to output"); return(0);  // JPH: changed this to invalid_argument exception
            }
        }
    };
    ...
    public:
        ...
        double p_T(double T) const{
            ...
            if ( ( T < Tmin ) || ( T > Tcrit ) ){
                raise_IF97duino_error("Temperature out of range"); return(0);
            }
            // Initial formulas
    ...
    namespace Backwards{
        ...
        public:
            ...
            virtual double T_pX(double p, double X) const{
                ...
                double summer = 0;
                for (std::size_t i = 0; i < N; ++i){
                    summer += n[i]*std::pow(p_i+a, I[i])*std::pow(eta+b, J[i])*std::pow(f, J[i]);
                }
...
//       It may double as Tsat(h,s) in Region 4 as well.
        ...
        inline double P2b2c_h(double h){
            ...
            const double p_i = region2b2c_n[0] + region2b2c_n[1]*eta + region2b2c_n[2]*eta*eta;
            return p_i*p_star;
        }
        inline double H2b2c_p(double p){
            // Only called for Region determination and debugging.  No range checking.
            const double p_star = 1*p_fact, h_star = 1*R_fact, p_i = p/p_star;
            const double ETA = region2b2c_n[3] + sqrt((p_i - region2b2c_n[4])/region2b2c_n[2]);
            return ETA*h_star;
        }
        ...
        inline double H3ab_p(double p){
            // Only called for Region determination and debugging.  No range checking.
            const double p_star = 1*p_fact, h_star = 1*R_fact, p_i = p/p_star;
            double ETA = region3ab_n[0] + region3ab_n[1]*p_i + region3ab_n[2]*p_i*p_i + region3ab_n[3]*p_i*p_i*p_i;
            return ETA*h_star;
        };
        ...
        inline double Hsat_s(double s){
            ...
            if (s < 0)
                raise_IF97duino_error("Entropy out of range");
            else if (s <= SfT23 )
                return b14hs.h_s(s);
            else if (s <= Scrit)
                return b3a4hs.h_s(s);
                ...
                raise_IF97duino_error("Entropy out of range"); return(0);
            }
        };
    ...
    inline IF97REGIONS RegionDetermination_TP(double T, double p)
    {
        ...
        if ((p < Pmin) || (p > Pmax)) {
            raise_IF97duino_error("Pressure out of range"); return(REGION_1);
        }

        // Now Check Temperature Range
        if (T > Text){
            raise_IF97duino_error("Temperature out of range"); return(REGION_1);
        }
    ...
    inline double RegionOutput(IF97parameters outkey, double T, double p, IF97SatState State){
        ...
        switch (region){
            ...
            case REGION_4: if (State == VAPOR) {
                               ...
                               raise_IF97duino_error("Cannot use Region 4 with T and p as inputs"); return(0);
                           }
                           break;
    ...
    inline IF97REGIONS RegionDetermination_pX(double p, double X, IF97parameters inkey){
        ...
        if (( X < Xmin ) || (X > (Xmax + 1.0E-10) )){
            if (inkey == IF97_HMASS){
                raise_IF97duino_error("Enthalpy out of range"); return(REGION_1);
            }
            else{
                raise_IF97duino_error("Entropy out of range"); return(REGION_1);
            }
        }
    ...
    inline int BackwardRegion(double p, double X, IF97parameters inkey){
        ...
        if ((inkey != IF97_HMASS) && (inkey != IF97_SMASS)) {
            raise_IF97duino_error("Backward Formulas take variable inputs of Enthalpy or Entropy only."); return(0);
        }
    ...
    inline double RegionOutputBackward(double p, double X, IF97parameters inkey, bool Clip, IF97SatState State) {
        ...
        //       this function limiits to IF97's Tmin and Tmax; aka, no clipp_ing.
        //
        // Make sure input and output keys are valid for Backward formulas
        if ((inkey != IF97_HMASS) && (inkey != IF97_SMASS)) {
            raise_IF97duino_error("Backward Formulas take variable inputs of Enthalpy or Entropy only."); return(0);
        ...
        double tmin = Tmin, tmax = Tmax;  // Initialize tmin, tmax as clipp_ing limits on sat. curve
...
//      case REGION_5: return 0; break;                       // No backward formulation for Region 5
    ...
    inline double Y_pX(IF97parameters outkey, double p, double X, IF97parameters inkey) {
        ...
        if ((inkey != IF97_HMASS) && (inkey != IF97_SMASS)) {
            ...
            raise_IF97duino_error("Reverse state cannot be determined for these inputs."); return(0);
        }
        ...
        const double T = RegionOutputBackward(p, X, inkey, false, NONE);  // no clipp_ing, no state specified
        ...
        switch (RegionDetermination_pX(p, X, inkey)) {
            case REGION_4: {                            // If in saturation dome (p <= Pcrit, Region 4)
                ...
                switch (outkey) {
                    ...
                    default:
                        raise_IF97duino_error("2-Phase: Requested output undefined in two-phase region."); return(0);
                        break;
                }
            ...
            default:  // all other regions
                raise_IF97duino_error("Reverse state functions not defined in REGION 5"); return(0);
                break;
        };
    }
        ...
        if ((p<Pmin) || (p>Pmax)) {
            ...
            switch (inkey) {
                ...
                default:
                    ...
                    raise_IF97duino_error("Quality cannot be determined for these inputs."); return(0);
            }
            if (t<Tcrit)
                return 0.0;  // Liquid, at all pressures above critical point
            else {
                // Supercritical Region (p>Pcrit) && (t>Tcrit)
                raise_IF97duino_error("Quality not defined in supercritical region."); return(0);
            }
        } else {                                                                    // p <= Pcrit
    ...
    inline double X_pQ(IF97parameters inkey, double p, double Q){
        ...
        if ((Q<0.0) || (Q>1.0)) {
            raise_IF97duino_error("Quality out of range"); return(0);
        }
        switch (inkey) {
            case IF97_HMASS:
            case IF97_SMASS:
            ...
            case IF97_DMASS:
                ...
                break;
            default: 
                raise_IF97duino_error("Mixture property undefined"); return(0);
                return -1;  // Should never occur but eliminates warnings.
                break;
        };
        return -1;  // Should never occur but eliminates warnings.
    ...
    inline IF97BACKREGIONS RegionDetermination_HS(double h, double s){
        ...
        if ( (s < Smin) || (s > Smax) ) {
            raise_IF97duino_error("Entropy out of range"); return(BACK_1);
        }
        if ( (h > Hmax(s)) || (h < Hmin(s)) ) {
            raise_IF97duino_error("Enthalpy out of range"); return(BACK_1);
        }
    ...
    inline double BackwardOutputHS(IF97parameters outkey,double h, double s){
        ...
        if ((outkey != IF97_P) && (outkey != IF97_T)) {
            raise_IF97duino_error("Backward HS Formulas output Temperature or Pressure only."); return(0);
        }
        ...
        case BACK_4:   if (s >= SgT23)   // T(h,s) only defined over part of the 2-phase region
                       ...
                       break;
        default: raise_IF97duino_error("Unable to match region"); return(0);
        }
...
/*  inline double Q_hsmass(double h, double s){
        ...
        if ((s < Smin) || (s > Smax)) {
            ...
            raise_IF97duino_error("Enthalpy out of range"); return(0);
        }
